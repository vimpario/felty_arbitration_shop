...
class BaseDAO(Generic[T]):
    ...
    async def delete(cls, session: AsyncSession, filters: BaseModel):
        ...
        if not filter_dict:
            logger.error(f"Нужен хотя бы один фильтр для удаления.")
            raise ValueError(f"Нужен хотя бы один фильтр для удаления")
        ...
        try:
            ...
            await session.rollback()
            logger.error(f"Ошибка при удлении записей: {e}")
            raise e
